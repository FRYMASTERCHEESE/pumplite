...
const identity = source.match(/declare_id!\("([^"]+)"\)/)[1];
assert.equal(idl.address, identity, 'IDL/build identity mismatch');
assert.ok(anchor.includes('pumplite = "' + identity + '"'), 'Anchor/build identity mismatch');
